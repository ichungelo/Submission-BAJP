...
class FavoriteMoviesFragment : Fragment(), SearchView.OnQueryTextListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        _favoriteMoviesViewModel = ViewModelProvider(this, factory)[FavoriteMoviesViewModel::class.java]
        _favoriteMoviesAdapter = FavoriteMoviesAdapter()

        favoriteMoviesViewModel?.getAllMoviesFavorite(EMPTY_SEARCH)?.observe(viewLifecycleOwner, { movies ->
    ...
    override fun onQueryTextChange(newText: String?): Boolean {
        ...
        if (!newText.isNullOrEmpty()) {
            favoriteMoviesViewModel?.getAllMoviesFavorite(searchQuery)?.observe(viewLifecycleOwner, { movies ->
                favoriteMoviesAdapter?.submitList(movies)
            })
        }
        return true
